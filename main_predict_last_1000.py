...
for t in tqdm(range(X.shape[0] - N_to_predict, X.shape[0], num_agents)):
    ...
    for k in range(num_agents):
        ...
        y_mean = edrfgp.ensemble_prediction_at_agent(
            k, x_agents[k]
        )  # each agent ONLY predicts at its arriving observations
    ...
    er_hat.append(
        [
            (agent_pred.squeeze() - y_agents[i].squeeze()) ** 2
...
avg_errs = np.mean(er_hat, axis=1)


# %%
np.savez(
    f"results/{args.dataset}_num_agents_{num_agents}_seed_{args.rand_seed}_last_1000.npz",
    avg_errs,
)
